...
[ApiController]
[Route("[controller]")]
public class LectureVoteController : ControllerBase
{
    private readonly ILectureVoteLogic _lectureVoteLogic;
    private readonly ILectureLogic _lectureLogic;
    private readonly IUserLogic _userLogic;

    public LectureVoteController(ILectureVoteLogic lectureVoteLogic, ILectureLogic lectureLogic, IUserLogic userLogic)
    {
    ...
    public async Task<ActionResult<LectureVoteDto>> CreateAsync([FromBody] LectureVoteDto dto)
    {
        try
        {
            ...
            return Created($"/lecturevote/{createdVoteDto.LectureUrl}", createdVoteDto);// ???
        }
        catch (Exception e)
        {
            Console.WriteLine(e);
